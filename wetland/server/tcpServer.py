...
class tcp_server(SocketServer.ThreadingTCPServer):
    ...
    def change_docker(self, new_addr):
        self.cfg.set("wetland", "docker_addr", new_addr)
...
class tcp_handler(SocketServer.BaseRequestHandler):
    def handle(self):
        ...
        sServer = sshServer.ssh_server(transport=transport, network=nw, cfg=self.server.cfg)
